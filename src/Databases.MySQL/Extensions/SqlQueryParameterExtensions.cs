...
namespace DBConfirm.Databases.MySQL.Extensions
{
    ...
    {
        /// <summary>
        /// Converts an array of <see cref="SqlQueryParameter"/> to an array of <see cref="MySqlParameter"/> objects
        /// </summary>
        /// <param name="parameters">The array to convert.  A null or empty array results in an empty array</param>
        /// <returns>Returns an array of <see cref="MySqlParameter"/> objects</returns>
        public static MySqlParameter[] ToSqlParameters(this SqlQueryParameter[] parameters)
        {
            ...
            return parameters.Select(p => new MySqlParameter($"@{p.ParameterName}", getValue(p))).ToArray();
        }
    }
}
