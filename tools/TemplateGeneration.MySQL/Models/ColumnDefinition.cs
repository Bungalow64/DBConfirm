...
namespace DBConfirm.TemplateGeneration.MySQL.Models
{
    public class ColumnDefinition
    {
        ...
        public ColumnDefinition(DataRow row)
        {
            ...
            IsNullable = row["IsNullable"].Equals(1);
            DefaultValue = row["DefaultValue"];
            DataType = row["DataType"].ToString();
            MaxCharacterLength = ToInt(row["MaxCharacterLength"]);
            IsIdentity = row["IsIdentity"].Equals(1);
            IsForeignKey = row["IsForeignKey"].Equals(1);
            ReferencedTableName = row["ReferencedTableName"]?.ToString();
            ReferencesIdentity = row["ReferencesIdentity"].Equals(1);
        }
